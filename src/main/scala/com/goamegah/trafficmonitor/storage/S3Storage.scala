...
object S3Storage extends StorageBackend {
  ...
  def store(data: String, prefix: String = "raw"): Unit = {
    ...
    try {
      ...
      println(s"[OK] Données stockées sur S3 : s3://$bucket/$key")
    } catch {
      case ex: Exception =>
        println(s"/!\\ Erreur lors du stockage S3 : ${ex.getMessage}")
    }
  }
  ...
  def retrieve(fileName: String, prefix: String = "raw"): Unit = {
    ...
    try {
      ...
      println(s"[OK] Données récupérées depuis S3 : s3://$bucket/$key")
      println(s"[DEBUG] Contenu : $data")
    } catch {
